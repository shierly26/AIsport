...
class agent:
	...
	def next_move(self, game_state, player_state):
		...
		# get list of ammo within range
		ammos_in_range = self.get_ammo_in_range(self.location, all_ammo)
		...
		# get occupied tile
		occupied_tiles = self.get_occupied_tiles(surrounding_tiles)
		...
		# if we're near a bomb, we should also probably move
			...
			if game_state.entity_at(self.location) == 'a':

				# get the ammo tile for us to move to
				ammo_tile = self.get_ammo_tile(occupied_tiles, ammos_in_range)	

				action = self.move_to_tile(self.location, ammo_tile)
	...
	def get_ammo_in_range(self, location, ammo):

		# empty list to store our ammos that are in range of us
		ammos_in_range = []

		# loop through all the ammos placed in the game
		for a in ammo:

			# get manhattan distance to a ammo
			distance = self.manhattan_distance(location, a)

			# set to some arbitrarily high distance
			if distance <= 10:
				ammos_in_range.append(a)

		return ammos_in_range
	...
	def get_occupied_tiles(self, tiles):

		# empty list to store our empty tiles
		occupied_tiles = []

		for tile in tiles:
			if self.game_state.is_occupied(tile):
				# the tile isn't occupied, so we'll add it to the list
	...
	def get_ammo_tile(self, tiles, ammo):

		# which bomb is closest to us?
		bomb_distance = 10  # some arbitrary high distance
		closest_ammo = bombs[0]

		for a in ammo:
			new_ammo_distance = self.manhattan_distance(ammo,self.location)
			if new_ammo_distance < ammo_distance:
				ammo_distance = new_ammo_distance
				closest_ammo = ammo
		...
		for tile in tiles:
			# get the manhattan distance
			distance = self.manhattan_distance(closest_ammo, tile)
			# store this in a dictionary
			ammo_dict[tile] = distance

		# return the tile with the furthest distance from any bomb
		ammo_tile = min(ammo_dict, key=ammo_dict.get)
